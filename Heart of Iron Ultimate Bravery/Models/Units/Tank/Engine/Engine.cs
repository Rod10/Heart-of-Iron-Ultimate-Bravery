...
public class Engine
{
    private readonly BaseEngine _implementation;

    public Engine()
    {
        _implementation = EngineFactory.CreateEngine();
    }
    
    // Method to get the actual implementation (for mod-specific operations)
    public T GetImplementation<T>() where T : BaseEngine
    {
        return _implementation as T;
    }

    // Method to access mod-specific properties safely
    public bool TryGetImplementation<T>(out T implementation) where T : BaseEngine
    {

...
public class Armor
{
    private readonly BaseArmor _implementation;

    public Armor()
    {
        _implementation = ArmorFactory.CreateArmor();
    }
    
    // Method to get the actual implementation (for mod-specific operations)
    public T GetImplementation<T>() where T : BaseArmor
    {
        return _implementation as T;
    }

    // Method to access mod-specific properties safely
    public bool TryGetImplementation<T>(out T implementation) where T : BaseArmor
    {
